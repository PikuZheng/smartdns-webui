...
import type { ServerError, TopDomainsBlocked } from '@/lib/backend/server';
...
export interface TopDomainsBlockedProps {
  domains: TopDomainsBlocked[];
  sx?: SxProps;
}

function TopDomainsBlockedTable({ domains }: TopDomainsBlockedProps): React.JSX.Element {
...
export function TopDomainsBlockedCard(sx: SxProps): React.JSX.Element {
  const { checkSessionError } = useUser();
  const [domains, setDomains] = React.useState<TopDomainsBlocked[]>([]);
  ...
  React.useEffect(() => {
    const fetchData = async (): Promise<void> => {
      const data = await smartdnsServer.GetTopDomainsBlocked();
      if (data.error) {
  ...
  return (
    <Box>
      <CardLoading title={t("Top Domains Blocked")} isLoading={loading} errorMsg={errorMsg}>
        <TopDomainsBlockedTable domains={domains} sx={sx} />
      </CardLoading>
    </Box>
  )
}
